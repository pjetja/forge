...
export const trainerTraineeConnections = pgTable(
...
export const schemaExercises = pgTable("schema_exercises", {
  ...
  schemaId: uuid("schema_id")
    .notNull()
    .references(() => workoutSchemas.id, { onDelete: "cascade" }),
  ...
  weightIncrementPerWeek: numeric("weight_increment_per_week", {
    precision: 5,
    scale: 2,
  }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
...
export const assignedPlans = pgTable("assigned_plans", {
  id: uuid("id").primaryKey().defaultRandom(),
  sourcePlanId: uuid("source_plan_id").references(() => plans.id, {
    onDelete: "set null",
  }),
  trainerAuthUid: uuid("trainer_auth_uid").notNull(),
  traineeAuthUid: uuid("trainee_auth_uid").notNull(),
...
export const assignedSchemas = pgTable("assigned_schemas", {
  ...
  assignedPlanId: uuid("assigned_plan_id")
    .notNull()
    .references(() => assignedPlans.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  slotIndex: integer("slot_index").notNull(),
  sortOrder: integer("sort_order").notNull().default(0),
});
...
export const assignedSchemaExercises = pgTable("assigned_schema_exercises", {
  id: uuid("id").primaryKey().defaultRandom(),
  assignedSchemaId: uuid("assigned_schema_id")
    .notNull()
    .references(() => assignedSchemas.id, { onDelete: "cascade" }),
  exerciseId: uuid("exercise_id")
    .notNull()
    .references(() => exercises.id, { onDelete: "restrict" }),
  sortOrder: integer("sort_order").notNull().default(0),
...
export type AssignedSchemaExercise =
  typeof assignedSchemaExercises.$inferSelect;
...
export const sessionSets = pgTable(
  "session_sets",
  {
    ...
    assignedSchemaExerciseId: uuid("assigned_schema_exercise_id")
      .notNull()
  ...
  (table) => ({
    uniqueSetPerSession: unique("session_set_unique").on(
      table.sessionId,
      table.assignedSchemaExerciseId,
      table.setNumber,
    ),
  }),
);
...
export type BodyWeightAccessRequest =
  typeof bodyWeightAccessRequests.$inferSelect;
