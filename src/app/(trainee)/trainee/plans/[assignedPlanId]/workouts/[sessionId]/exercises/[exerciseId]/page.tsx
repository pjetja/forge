...
export default async function ExerciseDetailPage({
  ...
  const { data: rawExercise } = await supabase
    ...
    .select(
      "id, exercise_id, sets, reps, target_weight_kg, per_set_weights, tempo, progression_mode, rpe_target, rir_target, weight_increment_per_week",
    )
    .eq("id", exerciseId)
    .single();
  ...
  const exerciseInfo = exerciseRow ?? {
    name: "Unknown",
    muscle_group: "",
    description: null,
    video_url: null,
      ...
      s.set_number,
      {
        ...
        actualWeightKg:
          s.actual_weight_kg != null
            ? parseFloat(String(s.actual_weight_kg))
            : null,
  ...
  if (completedSessionIds.length > 0) {
    ...
    const { data: samePrev } = (await supabase
      .from("session_sets")
      .select(
        "session_id, set_number, actual_reps, actual_weight_kg, muscle_failure",
      )
      .eq("assigned_schema_exercise_id", exerciseId)
    ...
    if (samePrev && samePrev.length > 0) {
      const mostRecentId = samePrev.reduce((best, s) =>
        (sessionRecency.get(s.session_id) ?? 999) <
        (sessionRecency.get(best.session_id) ?? 999)
          ? s
          : best,
      ).session_id;
  ...
  const perSetWeights = Array.isArray(ex.per_set_weights)
    ? (ex.per_set_weights as number[])
    : null;
  const targetWeightKgBase =
    ex.target_weight_kg != null
      ? parseFloat(String(ex.target_weight_kg))
      : null;
  ...
  const setRows: SetRow[] = Array.from({ length: ex.sets }, (_, i) => {
    ...
    return {
      ...
      actualWeightKg:
        ...
        (ex.progression_mode === "linear" &&
        ex.weight_increment_per_week != null &&
        lastSet?.actual_weight_kg != null
          ? Math.round(
              (parseFloat(String(lastSet.actual_weight_kg)) +
                ex.weight_increment_per_week) *
                4,
            ) / 4
          : targetWeightKg),
  ...
  const maxLoggedSetNumber =
    loggedSets.length > 0
      ? Math.max(...loggedSets.map((s) => s.set_number))
      : 0;
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-1">
          ...
          {ex.progression_mode && ex.progression_mode !== "none" && (
            <span className="text-xs text-accent border border-accent/30 rounded-sm px-1.5 py-0.5">
              {ex.progression_mode === "linear" &&
                t("workout.linearProgression")}
              {ex.progression_mode === "double_progression" &&
                t("workout.doubleProgression")}
      ...
      {exerciseInfo.video_url && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-text-primary opacity-60 uppercase tracking-wide">
            {t("workout.exerciseVideo")}
          </p>
          <div className="aspect-video w-full rounded-sm overflow-hidden bg-black">
            {exerciseInfo.video_url.includes("youtube.com") ||
            exerciseInfo.video_url.includes("youtu.be") ? (
              <iframe
