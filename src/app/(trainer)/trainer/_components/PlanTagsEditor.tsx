...
export function PlanTagsEditor({ planId, initialTags }: PlanTagsEditorProps) {
  ...
  return (
    <div className="space-y-1">
      <label className="text-sm font-medium text-text-primary">{t('planDetail.tags')}</label>
        ...
        {tags.map((tag) => (
          <span key={tag} className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-bg-page border border-border text-text-primary flex-shrink-0">
            ...
            <button
              ...
              aria-label={t('planDetail.removeTagAriaLabel', { tag })}
            >
              &times;
            </button>
          </span>
      ...
      <p className="text-xs text-text-primary opacity-40">{t('planDetail.tagsHint')}</p>
    </div>
  );
}
