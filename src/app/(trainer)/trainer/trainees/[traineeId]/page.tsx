...
export default async function TraineeDetailPage({
  ...
  const activeTab = ["exercises", "goals", "notes", "body-weight"].includes(
    resolvedSearch?.tab ?? "",
  )
  ...
  const { data: assignedPlans } = await supabase
    .from("assigned_plans")
    .select(
      "id, name, week_count, workouts_per_week, status, started_at, plan_updated_at, created_at",
    )
    .eq("trainee_auth_uid", traineeId)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <GravatarAvatar
          url={gravatarUrl(traineeProfile.email)}
        ...
        <div>
          ...
          <PhysicalStatsRow
            ...
            weightKg={
              traineeProfile.weight_kg ? Number(traineeProfile.weight_kg) : null
            }
      ...
      {activeTab === "plans" && (
        <>
          ...
          {(activePlan || pendingPlans.length === 0) && (
            <section>
              <h2 className="text-lg font-semibold text-text-primary mb-3">
                {t("traineeDetail.plans.currentPlan")}
              </h2>
              {activePlan ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-text-primary mt-1">
                        {t("traineeDetail.plans.weeks", {
                          count: activePlan.week_count,
                        })}{" "}
                        &middot;{" "}
                        {t("traineeDetail.plans.workoutsPerWeek", {
                          count: activePlan.workouts_per_week,
                        })}
                      </p>
                    </div>
          ...
          {pastPlans.length > 0 && (
            <section>
              <h2 className="text-lg font-semibold text-text-primary mb-3">
                {t("traineeDetail.plans.pastPlans")}
              </h2>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-text-primary opacity-60 mt-1">
                          {t("traineeDetail.plans.weeks", {
                            count: plan.week_count,
                          })}{" "}
                          &middot; {t("traineeDetail.plans.completed")}
                        </p>
                      </div>
                      <svg
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-text-primary opacity-60 mt-1">
                          ...
                          &middot; {t("traineeDetail.plans.terminated")}
                        </p>
                      </div>
                    </div>
                  ),
        ...
        (bodyWeightAccess === "approved" && weightEntries ? (
          ...
          <div className="space-y-3">
            ...
            {bodyWeightAccess === null && (
              <p className="text-sm text-text-primary opacity-60">
                {t("traineeDetail.bodyWeight.requestExplanation")}
              </p>
            )}
          </div>
        ))}
