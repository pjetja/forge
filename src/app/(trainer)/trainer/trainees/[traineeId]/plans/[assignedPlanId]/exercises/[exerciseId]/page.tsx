...
export default async function TrainerExerciseProgressPage({
  params,
}: TrainerExerciseProgressPageProps) {
  ...
  const { data: ase } = await supabase
    ...
    .select(
      "id, exercise_id, sets, reps, target_weight_kg, tempo, progression_mode, rpe_target, rir_target, weight_increment_per_week",
    )
    .eq("id", exerciseId)
    .single();
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {(exerciseInfo.muscle_group ||
          (ase as any).tempo ||
          ((ase as any).progression_mode &&
            (ase as any).progression_mode !== "none")) && (
        ...
        <p className="mt-1 text-sm text-text-primary opacity-60">
          ...
          {(ase as any).progression_mode === "rpe" &&
            (ase as any).rpe_target != null && (
              <> &middot; RPE {(ase as any).rpe_target}</>
            )}
          {(ase as any).progression_mode === "rir" &&
            (ase as any).rir_target != null && (
              <> &middot; RIR {(ase as any).rir_target}</>
            )}
          {(ase as any).progression_mode === "linear" &&
            (ase as any).weight_increment_per_week != null && (
              <>
                {" "}
                &middot; +
                {parseFloat(
                  String((ase as any).weight_increment_per_week),
                )}{" "}
                kg/week
              </>
            )}
      ...
      {chartData.length === 0 && (
        <div className="bg-bg-surface border border-border rounded-sm p-8 text-center">
          <p className="text-sm text-text-primary opacity-50">
            {t("traineeDetail.workout.noSets")}
          </p>
        </div>
      )}
    </div>
