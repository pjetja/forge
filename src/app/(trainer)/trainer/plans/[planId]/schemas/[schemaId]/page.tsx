  ...
  const items: SchemaExerciseItem[] = (schemaExercises ?? []).map((row) => {
    ...
    return {
      ...
      targetWeightKg: row.target_weight_kg
        ? parseFloat(String(row.target_weight_kg))
        : null,
      perSetWeights: Array.isArray(row.per_set_weights)
        ? (row.per_set_weights as number[])
        : null,
      tempo: (row as { tempo?: string | null }).tempo ?? null,
      progressionMode:
        (row as { progression_mode?: string | null }).progression_mode ??
        "none",
      rpeTarget: (row as { rpe_target?: number | null }).rpe_target ?? null,
      rirTarget: (row as { rir_target?: number | null }).rir_target ?? null,
      weightIncrementPerWeek: (
        row as { weight_increment_per_week?: string | null }
      ).weight_increment_per_week
        ? parseFloat(
            String(
              (row as { weight_increment_per_week?: string | null })
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text-primary opacity-60">
          {t("schemas.workoutLabel", { slot: schemaData.slot_index })}
        </p>
      </div>
